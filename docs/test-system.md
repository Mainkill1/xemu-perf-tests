...
- 144 executable leaves on this branch (the separately published ENG473 image
  contains the qualified 136-leaf subset);
- five non-measured groups;
- 149 exact legacy result aliases on this branch (141 in ENG473);
...
The current branch catalog contains 144 executable leaves and five structural
groups. The published ENG473 ISO remains a separately qualified 136-leaf,
141-record package; its count must not be confused with unbuilt branch
coverage. The generated catalog is the exhaustive per-ID list, including each
test's use. This table is the suite-level map.
