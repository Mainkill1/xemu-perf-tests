...
Source tracing points to `select_fragment_route_impl()`: on every dirty shader-state draw it hashes and probes the *full* specialized binding key before checking the fragment-module cache. A small follow-up candidate is to probe the narrower specialized fragment module first. If present, the normal binding path can look up/create the binding once; on a module miss it must still probe the binding because a live binding can retain a module after its module-cache entry has been evicted. This is a **hypothesis for a focused change and diagnostic**, not a measured fix. Preserve the existing fallback and retry behavior and require the normal-run correctness/performance gates before merging.
