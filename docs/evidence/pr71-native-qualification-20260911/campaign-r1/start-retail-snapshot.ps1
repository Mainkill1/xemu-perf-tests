...
$roamingRoot = Join-Path $profileRootFull 'Roaming'
$localRoot = Join-Path $profileRootFull 'Local'
New-Item -ItemType Directory -Path $roamingRoot, $localRoot -Force | Out-Null
$childEnvironment = @{
    APPDATA = $roamingRoot
    LOCALAPPDATA = $localRoot
    XEMU_VK_SHADER_IDENTITY_TRACE = ''
...
try {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -lt 7) {
            throw 'The private PR71 diagnostic launcher requires PowerShell 7 for explicit child environment.'
        }
        ...
        $profileActivationPath = Join-Path $captureDirectory 'profile-activation.json'
        [ordered]@{ schema_version = 1; process_id = $process.Id; explicit_start_process_environment = $true; appdata = $roamingRoot; localappdata = $localRoot; shader_identity_trace = 'disabled'; launched_utc = [DateTime]::UtcNow.ToString('O') } | ConvertTo-Json | Set-Content -LiteralPath $profileActivationPath -Encoding utf8
    } finally {
    ...
    [pscustomobject]@{
        ...
        Stdout = $stdoutPath
        Stderr = $stderrPath
        IsolatedProfileExplicit = $true
        ProfileActivationReceipt = $profileActivationPath
    }
} catch {
