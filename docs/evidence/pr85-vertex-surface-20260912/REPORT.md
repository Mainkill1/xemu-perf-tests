...
## PR #85 draft-review audit, 2026-09-12

The PR description was replaced by a 14,838-character source review without a new review comment or product commit. Its [exact captured text](review-pr85-body-20260912.md) is retained for attribution and follow-up; the product head remained `2163208fdc49c7f6b4834bce98e6a11b494d4241`. This audit checks source-level claims only, not a new runtime result.

| Review point | Disposition on the current head |
| --- | --- |
| Vertex fetch span uses `num_elements * stride` | **Confirmed.** `vk/vertex.c` queues that interval before calculating `element_size`, while CPU attribute decoding and Vulkan vertex fetch can read through the final element. For two 8-byte elements at stride 4, the queued span is 8 bytes but the enclosing span is 12; at stride 0, it is zero while the CPU reads an element. A stride larger than the element also includes trailing padding unnecessarily. |
| CPU attribute value decoded before readback | **Confirmed.** `pgraph_update_inline_value()` reads guest RAM in `pgraph_vk_bind_vertex_attributes()`; `sync_vertex_ram_buffer()` performs the potential overlapping-surface download only afterward. This can consume stale CPU RAM before the readback makes it current. |
| Shared dirty bit can be consumed by surface update | **Conditional.** `update_surface_part()` clears `DIRTY_MEMORY_NV2A` only when `!tcg_enabled()`. The reviewed Morrowind TCG path does not establish this loss. Preserve the concern for any supported non-TCG path or future ownership change; do not call it a reproduced Morrowind defect. |
| Readback marks `pitch * height` for a swizzled destination | **Confirmed layout gap.** The swizzle writer addresses `width * height * bytes_per_pixel` worth of destination data for supported power-of-two dimensions, while `download_surface()` marks `pitch * height`. When row bytes exceed pitch, the mark can omit the tail; the DMA assertion also validates only pitch-based size. The actual supported-layout and bounds contract needs a focused test. |
| Zero-length surface-overlap helper | **Confirmed edge case.** The half-open overlap predicate returns true for a zero-length range starting inside a surface. PR #85's vertex call guards `size &&`, so this is not evidence of a Morrowind hit. |

One pseudocode detail in the captured review must **not** be copied into code: `nv_dma_map()` returns `dma.limit` as `dma_len`, and the descriptor limit is **inclusive**. Requiring `fetch_size <= dma_len - start_offset` would reject an exact-fit request; with limit 15, offset 0, and 16 bytes, all bytes 0–15 are permitted. After checked start/extent arithmetic, the equivalent nonempty test is `start_offset <= limit && fetch_size - 1 <= limit - start_offset`, plus an independent VRAM-range check. The current `assert(attr->offset < dma_len)` is neither a full-span check nor correct for a valid single byte at the limit.

The first two points directly affect PR #85's stated vertex/surface coherency boundary and should be fixed before qualification. The swizzled footprint needs its own focused oracle. These findings do not change the existing Morrowind performance result or justify early report publication; the measured report/graphics-batch bottleneck remains tracked in issue #86 and PR #87.
