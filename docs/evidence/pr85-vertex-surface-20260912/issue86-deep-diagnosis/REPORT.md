...
The next narrow diagnostic is to classify why render passes end in this fixed workload: surface changes, nondraw commands, clears, query transitions, and finish. That targets the unexplained bind remainder without another broad benchmark. A pass-preservation patch is justified only for a class with correct output and a measured GPU cost. An earlier **ordered submission** remains a possible larger experiment, but `pgraph_vk_finish()` currently waits before recycling descriptor sets, staging offsets, framebuffers, and cache entries. Removing its fence wait alone would permit in-flight resource reuse; merely inserting another synchronous finish has already moved the wait without improving total throughput in the #85/#83 comparison. A safe overlapping design needs per-submission resource ownership and retirement, plus a completion wait before report publication. No pass-preservation or overlapping-submit product experiment has been implemented or qualified yet.
