...
The same probe provides a conservative bound on binds that are **not explained by a non-clear pipeline-key build or a clear draw**: `148,191 binds - 99,286 key builds - 1,216 clear draws = 47,689 binds`, or at least **32.2%** of binds (median **202** per guest display write). In `begin_draw()`, an absent render pass forces a bind even if the pipeline key did not change; that is the leading source-level explanation for this remainder. The counter does not record *why* each render pass ended, so 47,689 is **not** an exact pass-restart count. In the separate same-tree state-churn trace, Spearman rank correlation between binds and main-graphics GPU time was **0.502** across 229 measured writes; draw-command count was nearly fixed (median 1,151, p95 1,152) and its corresponding correlation was **0.003**. These associations prioritize pass-boundary attribution; they do not establish that removing a bind or pass boundary would save the correlated time.

### The render-pass breaker: dirty vertex-memory uploads

Two further **counter-only** builds on the same product tree classified the source path without changing renderer behavior. The pass-reason build used diagnostic patch SHA-256 `ee264b9af9aa8524e3e432ce8e1e7e3a1c9c6f2eb5c76ab590bac560e718a3b5`, executable SHA-256 `bfd7b815af7a068a9e239cb8fca802a455ddc029e4dae2b65a6230006b5a87cd`, and a 230-write Morrowind window. The vertex-reason build used patch SHA-256 `d20b9e98e2f16a511e73db27a37d24d415524146ac738344bb0e69b6e32ae9b3`, executable SHA-256 `365885cd0733251958f37f288c08631d6d7ed8bfa9523a70e199c43030f010e7`, and a 240-write window. Both used the same Windows O2/full-LTO/x86-v3 DWARF/assert profile and fixed snapshot recipe, reached gameplay, passed the final-image oracle, deleted the private disk, and closed xemu. They are attribution runs, **not** performance A/B cells.

| Measured counter | Pass-reason run | Vertex-reason run |
| --- | ---: | ---: |
| Render-pass begins / ends | 82,897 / 82,897 | 85,341 / 85,341 |
| Ends requested by nondraw commands | **78,962 (95.3%)** | **81,568 (95.6%)** |
| Ends from surface updates | 2,071 | 1,791 |
| Ends from clears | 1,404 | 1,502 |
| Ends at finish | 460 | 480 |
| Ends from framebuffer or query transitions | 0 | 0 |
| Vertex staging copies | 80,094 | 82,719 |
| Median vertex copies / nondraw pass ends per write | 353 / 348 | 353 / 348 |

The per-write counts of vertex staging copies and nondraw pass endings correlate at **0.999** in both runs. There were only **19** measured texture-upload calls in each window. Source tracing shows why these counts align: `sync_vertex_ram_buffer()` detects a dirty page-aligned vertex range and calls `pgraph_vk_update_vertex_ram_buffer()`; its active-command-buffer path calls `pgraph_vk_begin_nondraw_commands()`, which ends the current render pass, then records barriers and `vkCmdCopyBuffer()`. The next draw begins a pass and rebinds the graphics pipeline. The nondraw reason also includes other callers, so the counter is not an exact vertex-only pass-end count. [Pass-reason summary](pass-reasons-summary.json), [vertex-reason summary](vertex-reasons-summary.json), and their [run records](results/) retain the input hashes and exact identities.

The second build also checked **why** the vertex sync called the upload path. It saw **82,720 dirty-memory ranges**, **194,601 clean ranges**, and **zero** surface-overlap-only or dirty-plus-overlap ranges. There were **82,719** staging copies; one dirty range took the existing direct-copy path before a command buffer was active. Thus the repeated uploads in this scene are driven by the existing `DIRTY_MEMORY_NV2A` result, **not** by the conservative `surface_overlap` condition or staging-capacity fallback. Their total staged vertex payload was **725.2 MB over 10 seconds**, median **3.10 MB per guest display write**. Skipping those uploads because they are inconvenient would leave changed vertex data stale.

The renderer already merges adjacent vertex sync requirements within a draw. Since one dirty draw normally ends a pass once, merely batching the individual `vkCmdCopyBuffer()` calls inside that draw cannot eliminate the roughly 348 pass breaks per write. The [Vulkan copy-command rules](https://docs.vulkan.org/spec/latest/chapters/copies.html) require `vkCmdCopyBuffer()` outside a render pass; dynamic-rendering suspension also forbids intervening action commands. A real pass-preserving design must make the changed vertex bytes available to subsequent draws **without overwriting bytes earlier recorded draws still reference**. A bounded candidate is versioned, per-submission vertex ranges in a host-visible mapped ring, with draw bindings redirected to each immutable range and retirement after its fence. Measure host-visible vertex-fetch cost on the AMD test device as well as saved pass breaks and GPU time; keep the current staged device-local path as fallback. This is a design candidate, not an implemented or measured gain.
...
The first product experiment should now target the **dirty-vertex upload / render-pass break** path, preserving changed vertex bytes and ordering. Measure pass-end counts, main-graphics GPU time, total guest progression, image output, and frame tails against the exact parent and fixed baseline; an improvement in bind count alone is insufficient. Earlier **ordered submission** remains a larger, independent experiment. `pgraph_vk_finish()` currently waits before recycling descriptor sets, staging offsets, framebuffers, and cache entries. Removing its fence wait alone would permit in-flight resource reuse; merely inserting another synchronous finish has already moved the wait without improving total throughput in the #85/#83 comparison. A safe overlapping design needs per-submission ownership and retirement, plus completion before report publication. No product fix has been implemented or qualified by these diagnostics.
