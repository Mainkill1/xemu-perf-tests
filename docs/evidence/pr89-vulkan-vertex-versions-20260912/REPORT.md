...
The preliminary executable gave +0.30% Vulkan and +0.02% OpenGL suite medians, so the two same-runtime source revisions did not show a stable suite-wide gain. The final Vulkan queued-vertex-CPU-writes leaf improved **+39.79%** (83,722 → 50,411 µs) in the full-suite comparison, while Vulkan tiny inline arrays worsened **-15.21%** (37,964 → 43,740 µs). Those single-suite timing magnitudes did **not** repeat in the matched controls below. Large shifts also appeared in query and texture leaves outside this patch's direct path; for example, the Vulkan DMA-descriptor-rewrite leaf worsened -201.70% in the final run and -239.12% in the preliminary run. These isolated times require matched reproduction before assigning causality. The fenced S3TC streaming leaf changed from -9.58% preliminary to -1.13% final on Vulkan. The runner reported missing live markers, and candidate suite executions were not interleaved with the retained parent suite. [The 320-row final comparison](results/xiso-final-parent-comparison.csv) contains each test's outcome, comparable framebuffer hash, parent/candidate median µs, and Improvement %. [Preliminary comparison](results/xiso-preliminary-parent-comparison.csv) and [final suite receipt](results/xiso-final-receipt.json) are retained separately.

To check the direct path without another full-suite pass, the exact parent and final candidate then ran an **ABBA** control (`parent → candidate → candidate → parent`) for tiny inline arrays and the 11-leaf cross-title hot-path group. Each cell used the same image, catalog, renderer, scale, runner, five measurement-iteration multiplier, and one warmup iteration; the parent and candidate builds were uninstrumented. All eight cells passed, and every corresponding leaf kept the same outcome and framebuffer hash. These focused cells use a different invocation/iteration configuration from the full suite, so their absolute median µs should not be pooled with it.

| Matched Vulkan leaf | Raw + | Parent 1 → candidate 1 | Pair 1 Improvement % | Candidate 2 → parent 2 | Pair 2 Improvement % | Median-of-cells Improvement % |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Tiny inline arrays | `+bad` median µs | 71,057 → 68,696 | +3.32% | 70,151 → 64,470 | -8.81% | **-2.45%** |
| Queued vertex CPU writes | `+bad` median µs | 166,570 → 161,162 | +3.25% | 162,144 → 159,547 | -1.63% | **+0.86%** |

The order-reversed pairs change sign for both leaves. Thus the full-suite +39.79% targeted-leaf result and -15.21% tiny-inline result are **not stable estimates of patch effect**. Across the 11 hot-path leaves, the median-of-cells Improvement % is +1.40%, with mixed individual directions. This does not displace the two opposite-order Morrowind cadence pairs, whose fixed guest workload is a separate measurement. [All 12 focused leaf rows](results/focused-paired/comparison.csv) and [eight-cell receipt](results/focused-paired/receipt.json) retain the paired values and correctness checks. Query and texture leaf swings outside the direct vertex path remain unpaired.
...
This is one ordered pair, so the maximum and tail direction are not proven repeatable. [Candidate](results/pgr2-snapshot-final-candidate.json) and [parent](results/pgr2-snapshot-parent.json) summaries retain the interval tails. The exact-head suite does not establish an overall timing benefit. The repeated Morrowind cadence gain is the strongest performance evidence; the matched XISO hot-path group is modest and mixed. Keep #89 draft pending resolution of remaining material XISO timing swings and the parent #87/#85 correctness gates. No merge-ready claim is made.
