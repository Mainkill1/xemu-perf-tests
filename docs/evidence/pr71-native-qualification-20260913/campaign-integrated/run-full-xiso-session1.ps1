...
function Invoke-XisoCell(
    ...
    [ValidateSet('off', 'on')][string]$ShaderFastpath = 'off'
) {
    ...
    $arguments = @(
        ...
        '--comparison-previous-main-commit', '7a14b022baa1e81e6f2f4dd1ca1fda399a6916aa',
        '--run-order', $RunOrder,
        '--shader-cache', 'on',
        '--hybrid-ubershaders', $HybridUbershaders,
        '--shader-fastpath', $ShaderFastpath,
    ...
    try {
        ...
        $cell = [ordered]@{
            ...
            workload = if ($IncludeGated) {
                "full_xiso_all_$($Campaign.Xiso.RecordCount)_records"
            } else { "full_xiso_registered_$($Campaign.Xiso.RegisteredRecordCount)_records" }
            role = $Build.Role
            ...
            shader_fastpath = $ShaderFastpath
            run_order = $RunOrder
...
try {
    if (Test-Path -LiteralPath $root) {
        ...
        if ($existing.campaign_id -ne $Campaign.CampaignId -or
            $existing.status -ne 'failed' -or @($existing.cells).Count -gt 21 -or
    ...
    if (@($receipt.cells).Count -lt 11) {
        ...
        $portable['candidate-vulkan-on-fast'] = if (@($receipt.cells).Count -eq 0) {
            New-PortableBuild $Campaign $Campaign.Builds.candidate `
                'xiso-candidate-vulkan-on-fast' '' 'Enabled' 'On'
        } else {
            Get-PortableBuild $Campaign $Campaign.Builds.candidate `
                'xiso-candidate-vulkan-on-fast' 'Enabled' 'On'
        }
        $matrix = @(
        @('01-baseline-opengl', 'fixed_baseline', 'opengl', 'none', $false, 'off', 'baseline-candidate', 'fixed_baseline-opengl', 'off'),
        @('02-candidate-opengl', 'candidate', 'opengl', 'none', $true, 'off', 'candidate-baseline', 'candidate-opengl', 'off'),
        @('03-previous-opengl', 'previous_main', 'opengl', 'none', $true, 'off', 'baseline-candidate', 'previous_main-opengl', 'off'),
        @('04-previous-vulkan', 'previous_main', 'vulkan', 'none', $true, 'off', 'candidate-baseline', 'previous_main-vulkan', 'off'),
        @('05-candidate-vulkan-off-cold', 'candidate', 'vulkan', 'cold', $true, 'off', 'baseline-candidate', 'candidate-vulkan', 'off'),
        @('06-candidate-vulkan-off-warm', 'candidate', 'vulkan', 'warm', $true, 'off', 'candidate-baseline', 'candidate-vulkan', 'off'),
        @('07-candidate-vulkan-on-cold', 'candidate', 'vulkan', 'cold', $true, 'on', 'baseline-candidate', 'candidate-vulkan-on', 'off'),
        @('08-candidate-vulkan-on-warm', 'candidate', 'vulkan', 'warm', $true, 'on', 'candidate-baseline', 'candidate-vulkan-on', 'off'),
        @('09-baseline-vulkan', 'fixed_baseline', 'vulkan', 'none', $false, 'off', 'baseline-candidate', 'fixed_baseline-vulkan', 'off'),
        @('10-candidate-vulkan-fast-cold', 'candidate', 'vulkan', 'cold', $true, 'on', 'baseline-candidate', 'candidate-vulkan-on-fast', 'on'),
        @('11-candidate-vulkan-fast-warm', 'candidate', 'vulkan', 'warm', $true, 'on', 'candidate-baseline', 'candidate-vulkan-on-fast', 'on')
        )
        for ($matrixIndex = @($receipt.cells).Count;
            ...
            $receipt.cells += Invoke-XisoCell $entry[0] $Campaign.Builds[$role] `
                $portable[$entry[7]] $entry[2] $entry[3] $entry[4] $entry[5] $entry[6] $entry[8]
            Write-JsonAtomic $receiptPath $receipt
        }
    }
    $gatedIndex = 0
    $completedGatedCells = [math]::Max(0, @($receipt.cells).Count - 11)
