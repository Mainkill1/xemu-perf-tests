...
The owner then stopped further repeat testing after the first two clean PGR2 OpenGL cells. Their [partial retail table](../partial-retail/) is retained. The third cell was interrupted, its campaign receipt records failure with successful final cleanup, and its remaining PresentMon ETW session was explicitly closed. No exact-head Vulkan retail or Morrowind performance result is claimed from this stopped campaign.
