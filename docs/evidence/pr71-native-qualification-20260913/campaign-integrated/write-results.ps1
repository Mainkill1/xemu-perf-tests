...
$xisoRows = @($xiso.cells | ForEach-Object {
    ...
    [pscustomobject][ordered]@{
        ...
        Fastpath = if ($gated) { 'off' } else { $_.shader_fastpath }
...
Write-MarkdownTable (Join-Path $outputRoot 'full-xiso.md') `
    ...
    @('Role','Renderer','Hybrid','Fastpath','Cache','Records','Non_PASS','Functional_hash','Validation',
...
$requiredWarmCells = 2 * @($retail.cells.workload | Sort-Object -Unique).Count
$cachePass = $warmProof.Count -ge $requiredWarmCells -and @($warmProof | Where-Object {
